...
Для автоматической сборки и запуска воспользуйтесь скриптом `build_and_run.bat`:

1. Убедитесь, что установлен Python 3.12+ и доступна команда `py`.
2. Запустите файл `build_and_run.bat` двойным кликом или из командной строки.
   Скрипт автоматически переключит кодовую страницу консоли в UTF-8, чтобы русскоязычные подсказки отображались корректно,
   и по завершении вернёт предыдущее значение.
3. Введите запрашиваемые значения (Aspose Client ID/Secret, при необходимости секретный ключ и порт).
4. Скрипт создаст виртуальное окружение, установит зависимости, сохранит параметры в `.env`, запустит приложение и откроет страницу в браузере.
5. Полный вывод работы скрипта автоматически сохраняется в файл `logs\build_and_run_YYYY-MM-DD_HH-mm-ss.log` (UTF-8), что позволяет быстро изучить журнал при возникновении ошибок.
6. При сбое окно консоли не закроется сразу: скрипт выведет код ошибки, напомнит, где найти лог, и дождётся нажатия Enter.
